...
const Weather: React.FC<WeatherProps> = ({ currentLocation }) => {
  ...
  if (status === "failed") {
    return (
      <div style={styles.floatingContainer}>Failed to load weather data</div>
    );
  }
  ...
  const chartData = {
    ...
    datasets: [
      ...
      },
      {
        ...
        data: weather.map((item) => item.values.precipitationProbability),
  ...
  return (
    <div style={styles.floatingContainer}>
      ...
      <div
        style={visible ? styles.hidePanelBtnVisible : styles.hidePanelBtnHidden}
      >
        <a onClick={() => setVisible(!visible)} style={styles.hidePannelLink}>
          {visible ? "HIDE" : "SHOW"}
        </a>
      </div>
      <div
        style={
          visible ? styles.chartContainerVisible : styles.chartContainerHidden
        }
      >
        <Line
          data={chartData}
